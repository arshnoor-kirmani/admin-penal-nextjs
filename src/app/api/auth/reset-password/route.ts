...
export async function POST(req: Request) {
  try {
    ...
    if(!identifier || !userType || !password) {
      return NextResponse.json({ message: "Identifier, userType and password are required" }, { status: 400 });
    }
    console.log({"identifier": identifier, "userType": userType, "password": password});
    let user;
